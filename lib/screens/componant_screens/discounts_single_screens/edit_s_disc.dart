...
class _UpdateSingleDiscScreenState extends State<UpdateSingleDiscScreen> {
  ...
  //update tax
  updateSingleDiscFn(BuildContext context, int id) {
    final db = DatabaseCubit.dbFrom(context);
    db.discSingleRepo.editSingleDisc(
      model: SingleDisc(
        id: widget.item.id,
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Edit single coupon'),
        ),
            ...
            onPressed: () {
              if (_formKey.currentState!.validate()) {
                if (validFromDate != null) {
                  updateSingleDiscFn(context, widget.item.id);
                } else {
