...
class WorkerApiRepoImpl implements WorkerApiRepo {
  ...
  Future<Either<Failure, List<WorkersModel>>> getWorkers() async {
    try {
      ...
      final List<WorkersModel> workers =
          data.map((e) => WorkersModel.fromMap(e)).toList();
  ...
  @override
  Future<Either<Failure, List<WorkersModel>>> getWaiters() async {
    try {
      final response =
          await Helpers.sendRequest(RequestType.get, EndPoints.getWaiters);
      final List<dynamic> data = response?['data'];
